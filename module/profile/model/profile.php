...
class Profile_Model_Profile extends Linko_Model
{
    ...
    public function set($sUser)
    {
        ...
        {
            //register menu
            $this->registerMenu(Lang::t('profile-activity-title'), array('link' => $this->buildUrl().'/activity','id' => 'activity'));
        }

        $sModule = strtolower(Linko::Module()->getSetting('profile.default_profile_module'));
    ...
    public function setController($sSlug)
    {
       //s echo $sSlug;
        ...
        $currentSlug = explode('/', $sSlug);
        $this->_defaultProfileSlug = $currentSlug[0];
    ...
    public function getDefaultSlugModules()
    {
        $aModules = array('info' => 'User Basic Information');

        if(Linko::Module()->isModule('activity'))
        {
            $aModules['activity'] = 'User Activity ';
        }

        //arr::dump(Linko::Module()->getModules());
        foreach(Linko::Module()->getModules() as $sModule => $val)
        {
            if(Linko::Module()->hasTask($sModule, 'profile_default_slug'))
            {
                $aModules[$sModule] = ucwords($sModule.' Module');
            }
        }
        return $aModules;
    }
    ...
    {
        return $this->_defaultProfileSlug;
    }
