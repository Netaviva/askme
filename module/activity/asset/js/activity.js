		...
		if(e.keyCode == 13 && !e.shiftKey) {
			...
			Linko.ajax.post('activity/addComment', param, function(response) {
				...
				if(response) {
					...
					$Activity.resetTextarea($textarea);
				}
			});
		}
	});
...
$Activity.autoResizeText = function(){
	$('.activity-comments textarea').keydown(function(){
		Linko.util.resizeTextarea($(this));
	});
};

$Activity.resetTextarea = function($s){
	console.log($s);
	$s.height($TextAreaHeight);
}
