...
(function(win, $) {
		...
		init:function() {
			...
			$("[data-modal]").each(function(i) {
				var $content = $('#' + $(this).attr('data-modal') + '-modal'),
					$self = $(this), options = {};
				$content.hide();
				options.type = $content.attr('data-type') || 'content';
				options.title = $content.attr('title');

				if(options.type == 'form') {
					options.action = $content.data('action') || location.hash;
					options.method = $content.data('method') || 'post';
	...
	Linko.modal = {
		...
		show:function(data, options) {
			...
			// set default options
			options = $.extend({
				type:'content', // content | form
				action:'', // used in form
				method: 'post', // used in form
			...
			var callback, html = this[options.type].template.replace('{%content%}', data)
				.replace('{%action%}', options.action)
				.replace('{%title%}', options.title)
				.replace('{%method%}', options.method)
				.replace('{%submit%}', options.submit);

			callback = options.callback;

			// if the callback is a string, then its probably a global function defined somewhere...
			// i trust you have done so
			if(typeof callback == 'string'){
				callback = window[options.callback];
			}

			// if its not a function,  set a default callback
			// ok. talking about trust... we just have to do things ourselves.. sorry!
			...
			options = $.extend({
				...
				containerId: (options.type == 'form' ? 'modal-form-container' : 'modal-content-container'),
				minHeight:260,
				minWidth:520,
				overlayClose:true,
				autoResize:true,
				onShow:function(modal) {
					...
					if(options.type == 'form') {
						if(options.action.substr(0, 7) != 'http://') {
							modal.container.find('form').on('submit', function(e) {
								var param = $.param(
									$(this).formToArray()
								);
								Linko.ajax.call(options.action, param, function(response) {
									callback.call(modal, response);
									window.setTimeout(function() {
										Linko.modal.close();
									}, options.autoCloseInterval);
								}, options.method, options.datatype);
	...
	Linko.process = {
		...
		show:function(id, elem, options) {
			...
			var $template = $('<span class="loading-notification ' + id + '"><img src="' + options.image + '" /></span>');

			this.store[id] = $template[options.method](elem);

			if(options.center) {
				$template.css({
					display:'block',
					marginTop:(elem.height() / 2) - ($template.height() / 2),
					textAlign:'center'
				});
			}
		},
